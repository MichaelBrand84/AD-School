...
Falls du dich mit Fehlerbehandlung (*Exceptions*) auskennst, dann soll deine Funktion einen `ZeroDivisionError` werfen, wenn durch Null geteilt wird.
...
Ergänze die Datei `floatsad.py` mit der Methoden `__pow__(self, other)`. Dabei übernimmt `self` die Rolle von $v_1$ in der obigen Herleitung und `other` entspricht $v_2$. Die Funktion $\ln(\ldots)$ heisst in Python `math.log()`.
Falls du dich mit *Exceptions* auskennst, dann soll deine Funktion einen `ValueError` werfen, wenn durch $v_1 \le 0$ ist.
...
Ergänze die Datei `floatsad.py` mit der Methoden `__rpow__(self, other)`. Implementiere wenn möglich auch die Fehlerbehandlung.
Teste dein Programm an verschiedenen Funktionen.
