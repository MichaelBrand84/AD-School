...
Implementiere auf die gleiche Weise den `-`-Operator. Die Methoden lauten `__sub__` bzw. `__rsub__`.
Schreibe auch eine Testfunktion `f`, welche die neuen Operatoren verwenden.
...
Ergänze die Datei `floatsad.py` mit den Methoden `__mul__` und `__rmul__`. 
