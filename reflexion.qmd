...
Da noch nicht alle Schüler:innen mit Python programmiert hatten, folgte als nächstes eine kurze Einführung in die Syntax. Als Programmierumgebung habe ich [Codeboard.io](https://codeboard.io/) verwendet, welches im Browser verwendet werden kann und im Gegensatz zu [TigerJython](https://tigerjython.ch/de) auch `numpy` und `matplotlib` unterstützt. Bei letzterer muss allerdings die Ausgabe als Bild abgespeichert werden und dann auf der Konsole ausgedruckt werden. Das folgende einfache Beispiel verdeutlicht das Vorgehen:
```{python}
#| code-fold: false
#| eval: false
import matplotlib
