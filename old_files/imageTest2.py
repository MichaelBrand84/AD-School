# Bild inkl. Farbe
# Problem: Rekursionstiefe wird zu hoch
...
def loss(x):
    ...
    [length, width, col] = np.shape(x)
    temp = blur(x)
    temparray = np.reshape(temp, length * width * col)
...
image = original
[length, width, col] = np.shape(image)

# Blurr erzeugen
blurred = blur(image)
blurrarray = np.reshape(blurred, length*width*col)
blurrarray = float2FloatAad(blurrarray)

# Startwert
guessimage = np.full(shape = [length, width, col], fill_value=0.5)
...
while lossval > tol:
    [lossval, grad] = loss(guessimage) 
    diff = np.reshape(grad, [length, width, col])
