...
Schreibe ein Programm, das mit Hilfe des Newtonverfahrens eine Nullstelle der Funktion $f(x) = \frac{1}{31} x^3 -\frac{1}{20} x^2 -x + 1$ berechnet. Verwende den Startwert $x_0 = -2$. Du kannst abbrechen, wenn die Differenz $|x_{n+1} - x_n|$ kleiner als eine bestimmte Toleranz wird, z.B. kleiner als `tol = 1e-6`.
...
print(xbar)
```
Das Newtonverfahren wird als eigene Funktion `newton(f, fdot, x0)` implementiert. Dieser werden die Funktion $f$ und ihre Ableitung $f'$, sowie der Startwert $x_0$ als Argumente übergeben werden. Sie kann dann im Hauptprogramm aufgerufen werden. Die Ableitung wurde aber immer noch von Hand berechnet.
...
Das folgende Programm berechnet das Skalarprodukt dieser beiden Vektoren.

```{python}
#| code-fold: show
#| label: fig-GraphOfBillard
#| fig-cap: "Graph des Skalarprodukts als Funktion des Polarwinkels $x$."
...
def f(x):
    ...
    px, py = 0.6, 0.7  # Position von P = (px|py)
